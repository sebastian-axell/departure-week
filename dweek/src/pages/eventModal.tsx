...
export default function EventModal({ handleOnClick, currentEvent, setEvents, events }: { handleOnClick: () => void, currentEvent: Event, setEvents: any, events: any }) {
    ...
    async function sendEmail(object: any, interestee: People | null) {
    ...
    const toggleInterestee = (person: People) => {
        if (person === interestee) {
            setButtonDisabled(true)
            return setInterestee(null)
        }
        const index = attendees.indexOf(person)
        if (index > -1) {
            setButtonDisabled(false);
            const filteredList = attendees.filter((obj: any) => obj !== person)
            setAttendees(filteredList);
            const tempEvent: Event = {
                ...currentEvent,
                attendees: filteredList
            }
            let filteredEvents = events.filter((event:any) => event.title !=currentEvent.title)
            filteredEvents = [tempEvent, ...filteredEvents]
    ...
    return (
        <div className="z-10 h-screen w-screen bg-gray-300/50 flex fixed justify-center items-center top-0 left-0 right-0 font-semibold text-black">
            <div className="p-4 bg-contrast bg-white shadow rounded-lg border-2 border-black w-10/12 sm:w-8/12 h-fit md:w-6/12 lg:w-4/12 max-w-xl relative">
                <div onClick={() => handleOnClick()} className="absolute uppercase right-3 top-0 p-1">x</div>
                    ...
                    <div className="grid grid-cols-3 w-full lg:w-10/12 p-3 gap-3">
                        {
                            (Object.values(People) as People[]).map((person) => (
                                <button onClick={() => toggleInterestee(person)}
                                    className={`rounded-lg w-fit mx-auto px-3 py-1 ${interestee === person ? "bg-yellow-400" : "bg-gray-200"} ${attendees.indexOf(person) > -1 ? "bg-yellow-400" : ""}`} key={person}>{person}</button>
                            ))
                        }
                    </div>
                    {
                        wait ?
                            ...
                            <button disabled={buttonDisabled} onClick={
                                () => {
                                    setWait(true);
                                    sendEmail(currentEvent, interestee);
                                }}
                                className={`px-2 py-1 text-xl border-2 w-full mx-auto border-black rounded-xl bg-accent tracking-widest disabled:opacity-50`}>
                                {
                                    success ?
                                        "interest sent!"
                                        :
