...
export default function Modal({ handleOnClick, currentThing, people }: { handleOnClick: () => void, currentThing: object, people: People }) {
                    ...
                    <div className="grid grid-cols-3 w-full lg:w-10/12 p-3 gap-3">
                        {
                            (Object.values(People) as People[]).map((person) => (
                                <button onClick={() => toggleInterestee(person)} className={`rounded-lg w-fit mx-auto px-3 py-1 ${interestee === person ? "bg-yellow-400" : "bg-gray-200"}`} key={person}>{person}</button>
                            ))
                        }
                    </div>
                    {
